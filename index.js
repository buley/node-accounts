    ...
    if (params && 'logout' === params.action) {
        ...
        service.vimeo.getOAuthAccessToken(params.code, {
            ...
            if (error) {
                ...
                service.vimeo.get("https://api.vimeo.com/me", access_token, function(error, data, response) {
...
Private.handle_facebook = function(params, callback) {
    ...
    if (params && 'connect' === params.action) {
        ...
        if (params.error_reason == 'user_denied') {
            ...
            service.facebook.getOAuthAccessToken(params.code, {
...
Private.handle_github = function(params, callback) {
    ...
    if (params && 'connect' === params.action) {
        ...
        if (params.error_reason == 'user_denied') {
            ...
            service.github.getOAuthAccessToken(params.code, {
                'grant_type': 'authorization_code'
            }, function(error, access_token, refresh_token) {
...
Private.handle_soundcloud = function(params, callback) {
    ...
    if (params && 'proxy' === params.action) {
        ...
        if (params.error_reason == 'user_denied') {
            ...
            service.soundcloud.getOAuthAccessToken(params.code, {
                'grant_type': 'authorization_code',
                redirect_uri: soundcloud.callback
            }, function(error, access_token, refresh_token) {
        ...
        var redirect_url = service.soundcloud.getAuthorizeUrl({
            redirect_uri: soundcloud.callback,
            response_type: "code"
        });
...
Public.prototype.secrets = function(path) {
    ...
    // Github (oAuth 2)
    if (!!github) {
        service.github = new OAuth2(github.id, github.secret, "https://github.com", "/login/oauth/authorize", "/login/oauth/access_token", {
            'User-Agent': 'republish.co (@editor)'
        });
    ...
    // Linkedin (oauth 1.0)
    if (!!linkedin) {
        service.linkedin = new OAuth("https://api.linkedin.com/uas/oauth/requestToken", "https://api.linkedin.com/uas/oauth/accessToken", linkedin.id, linkedin.secret, "1.0", linkedin.callback || null, "HMAC-SHA1", null, {
            'x-li-format': 'json'
        });
    }
...
Public.prototype.proxy = function(data, callback) {
    ...
    } else if ('soundcloud' === data.service) {
        Private.handle_soundcloud(data, callback);
    }
};


Public.prototype.login = function(data, callback) {
    Private.publish('logging_in', {
        'data': data,
        'callback': callback
    });
    if ('twitter' == data.service) {
