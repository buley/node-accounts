				...
				if( error ) { 
					...
					service.facebook.get( "https://graph.facebook.com/me", access_token, function ( error, data, response ) {
						if( error ) {
							...
							var result = { 'success': false, 'account_status': 'unauthorized', 'service': 'facebook' };
							callback( result );
							
						} else {

							var parsed = JSON.parse( data );
							var result = { 'success': true, 'account_status': 'authorized', 'service': 'facebook', 'access_token': access_token, 'profile_data': parsed };
