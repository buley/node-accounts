    ...
    if( params && 'logout' == params.command ) {
        ...
        service.vimeo.getOAuthAccessToken( params.oauth_token, params.oauth_token_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
            ...
            if( error ) {
                ...
                service.vimeo.get( "http://vimeo.com/api/rest/v2?format=json&method=vimeo.test.login", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {

                    var parsed = JSON.parse( data )
                        , user = null;
                    console.log('VIMEO',parsed);

                    if( error || 'undefined' === typeof parsed.user ) {
                        ...
                        user = parsed.user;
                        service.vimeo.get( "http://vimeo.com/api/rest/v2?format=json&method=vimeo.people.getInfo&user_id=" + user.id, oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
