	...
	if( params && 'logout' === params.action ) {
		...
		service.linkedin.getOAuthRequestToken( function( error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters ) {
			...
			if( error ) {
				...
				var authentication_url = "https://www.linkedin.com/uas/oauth/authenticate?oauth_token=" + oauth_request_token + "&scope=" + encodeURIComponent( exports.linkedin.scope );
    ...
    if( params && 'connect' === params.action ) {
        ...
        service.reddit.get( "https://oauth.reddit.com/api/me.json", params.access_token, function ( error, data, response ) {
            if( error ) {
				...
				service.reddit.get( "https://oauth.reddit.com/reddits/mine/subscriber.json?scope=mysubreddits", params.access_token, function ( error, data, response2 ) {

					console.log("REDDIT RESPONSE", response2.response );
					Private.publish( 'authorized', {
						'data': params
						, 'callback': callback
						, 'data': profile
    ...
    if( params && 'logout' === params.action ) {
        ...
        if ( 'GET' === method ) {
            ...
            service.vimeo.get( url, oauth_token, oauth_token_secret, function (error, data, result ) {
				console.log("VIMEO", error, data, result );
                if( error ) {
					success = false;
                }
                try {
