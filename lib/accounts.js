	...
	if( params && 'logout' == params.command ) {
		...
		service.yahoo.getOAuthRequestToken( function( error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters ) {
			...
			if(error) {
				...
				var authentication_url = "http://yahoo.com/oauth/authorize/?oauth_token=" + oauth_request_token;
...
Private.handle_github = function( params, socket, type ) {
	...
	if( params && 'connect' == params.command ) {
		...
		service.github.getProtectedResource( "https://api.github.com/v2/users/self", params.access_token, function ( error, data, response ) {
				if( error ) {
		...
		if( params.error_reason == 'user_denied' ) {
			...
			service.github.getOAuthAccessToken( params.code, { redirect_uri: githubCallbackAddress, 'grant_type': 'authorization_code' }, function( error, access_token, refresh_token ){
				...
				if( error ) { 
					...
					service.github.getProtectedResource( "https://api.github.com/v2/users/self", access_token, function ( error, data, response ) {
...
Public.prototype.secrets = function( path ) {
	...
	// Yahoo (oauth 1.0)
	service.yahoo = new OAuth( "https://api.login.yahoo.com/oauth/v2/get_request_token", "https://api.login.yahoo.com/oauth/v2/get_token", tumblrId, tumblrSecret, "1.0", yahooCallbackAddress || null, "HMAC-SHA1" );
