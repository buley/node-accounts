...
Private.handle_tumblr = function( params, callback ) {
	...
	if( params && 'logout' == params.command ) {
		...
		service.tumblr.get( "http://www.tumblr.com/oauth/authorize", params.access_token, params.access_token_secret, function (error, data, result ) {
			console.log("TUBMLR", error, data, result);	
			if( error ) { 
		...
		params.oauth_verifier = params.oauth_verifier.replace( /#_=_/, '' )
		service.tumblr.getOAuthAccessToken( params.oauth_token, params.oauth_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {

			console.log('TUMBLR RESPONSE',error,oauth_access_token, oauth_access_token_secret, additionalParameters);
...
Public.prototype.secrets = function( path ) {
	...
	// Tumblr (oauth 1.0)
	console.log('creating T', tumblr.id, tumblr.secret, tumblr.callback);
