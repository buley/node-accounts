...
Private.handle_reddit = function( params, callback ) {
    ...
    if( params && 'connect' == params.command ) {
        ...
        service.reddit.get( "https://public-api.reddit.com/rest/v1/me", params.access_token, function ( error, data, response ) {
            if( error ) {
        ...
        if( params.error_reason == 'user_denied' ) {
            ...
            console.log('getting reddit access token', params.code);
            service.reddit.getOAuthAccessToken( params.code, {
                'grant_type': 'authorization_code'
                , redirect_uri: reddit.callback
            }, function( error, access_token, refresh_token ){
                console.log('ACECSS TOKEN',error,access_token,refresh_token)
                if( error ) {
                    ...
                    console.log('getting me for reddit', access_token);
                    service.reddit.get( "https://public-api.reddit.com/rest/v1/me", access_token, function ( error, data, response ) {
                        console.log('ACECSS TOKEN',error,access_token,data,response);
                        if( error ) {
...
Public.prototype.secrets = function( path ) {
    ...
    service.reddit = new OAuth2( reddit.id, reddit.secret, "https://ssl.reddit.com", "/api/v1/authorize", "/api/v1/access_token", {
        'Authorization': "Basic " + Buffer("" + reddit.id + ":" + reddit.secret).toString('base64');
    } );
