...
Private.handle_tumblr = function( params, callback ) {
		console.log('HANDLING TUMBLR', params );
	if( params && 'logout' === params.action ) {
	...
	} else if( params && 'sign' === params.action ) {

		Private.publish( 'sign', {
			'data': params
			, 'callback': callback
		} );
		...
		var oauth_token = params.oauth_token
			...
			, oparams = service.tumblr._prepareParameters( oauth_token, oauth_token_secret, method, url, extra_params )
			, authorization = service.tumblr._buildAuthorizationHeaders( oparams );

		service.tumblr.get( url, params.access_token, params.access_token_secret, function (error, data, result ) {

            if( error ) {
				console.log("TUMBLR ERROR", error );
			} else {
				console.log("TUMBLR DATA", data );
			}
...
Private.handle_twitter = function( params, callback ) {
	...
	if( params && 'logout' === params.action ) {
		...
		var oauth_token = params.oauth_token
			...
			, url = params.url || ''
			, extra_params = params.params || {}
			, oparams = service.twitter._prepareParameters( oauth_token, oauth_token_secret, method, url, extra_params )
			, authorization = service.tumblr._buildAuthorizationHeaders( oparams );
...
Public.prototype.sign = function( data, callback ) {
	Private.publish( 'signing', { 'data': data, 'callback': callback } );
