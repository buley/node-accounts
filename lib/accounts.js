    ...
    if( params && 'connect' == params.command ) {
        ...
        if( params.error_reason == 'user_denied' ) {
            ...
            service.instagram.getOAuthAccessToken( params.code, {
                ...
                console.log('got instal (' + instagram.id+')',error, access_token, refresh_token );
                if( error ) {
                    ...
                    console.log('no insta err so getting', access_token);
                    service.instagram.get( "https://api.instagram.com/v1/users/self", access_token, function ( error, data, response ) {

                        if( error ) {
                            console.log('OINSTA self error');
                            Private.publish( 'unauthorized', {
                                'data': params
                                , 'callback': callback
                                , 'error': error
...
Public.prototype.secrets = function( path ) {
    ...
    // Instagram (oAuth 2)
    service.instagram = new OAuth2( instagram.id, instagram.secret, "https://api.instagram.com", "/oauth/authorize", "/oauth/token" );
