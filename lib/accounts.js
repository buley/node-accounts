...
Accounts.handle_evernote = function( params, socket ) {
	...
	if( params && 'logout' == params.command ) {
		...
		Accounts.publish( 'deauthorized', { 'data': params, 'socket': socket, 'type': type } );
		...
		service.evernote.get( "https://www.evernote.com/oauth", params.access_token, params.access_token_secret, function (error, data, result ) {
			...
			if( error ) { 
				...
				Accounts.publish( 'unauthorized', { 'data': params, 'socket': socket, 'type': type, 'error': error } );
				...
				Accounts.publish( 'authorized', { 'data': params, 'socket': socket, 'type': type, 'response': parsed } );
		...
		Accounts.publish( 'unauthorized', { 'data': params, 'socket': socket, 'type': type } );
		...
		service.evernote.getOAuthAccessToken(params.oauth_token, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				service.evernote.get( "https://www.evernote.com/oauth", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
					...
					Accounts.publish( 'authorized', { 'data': params, 'socket': socket, 'type': type, 'response': parsed, 'access_token': oauth_access_token, 'access_token_secret': oauth_access_token_secret } );
		...
		service.evernote.getOAuthRequestToken(function(error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters ) {
			if(error) {
				...
				var authentication_url = "http://evernote.com/oauth/authenticate?oauth_token=" + oauth_request_token;

				Accounts.publish( 'authorizing', { 'data': params, 'socket': socket, 'type': type, 'authorization_url': authentication_url  } );	
				socket.emit( 'response', {
					'success': true
					, 'response_type': 'account'
					, 'service': 'evernote'
...
Accounts.handle_twitter = function( params, socket, type ) {
	...
	if( params && 'logout' == params.command ) {
		...
		Accounts.publish( 'deauthorized', { 'data': data, 'socket': socket, 'type': type } );
		...
		service.twitter.get( "https://api.twitter.com/1/account/verify_credentials.json", params.access_token, params.access_token_secret, function (error, data, result ) {
			...
			if( error ) { 
				...
				var parsed = JSON.parse( data );
				Accounts.publish( 'authorized', { 'data': params, 'response': parsed, 'socket': socket, 'type': type } );
		...
		service.twitter.getOAuthRequestToken(function(error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters ) {
			...
			if(error) {
				...
				socket.emit( 'response', { 'success': true, 'response_type': 'account', 'service': 'twitter', 'login_url': authentication_url, 'request_token': oauth_request_token, 'request_token_secret': oauth_request_token_secret } );
...
Accounts.handle_facebook = function( params, socket ) {
	...
	if( params && 'connect' == params.command ) {
		...
		Accounts.publish( 'authorizing', { 'data': params, 'socket': socket, 'type': type, 'authorization_url': redirect_url } );
	...
	if( params && 'connect' == params.command ) {
		...
		service.google.get( "https://www.googleapis.com/plus/v1/people/me", params.access_token, function( error, data ) {
			...
			if( error ) {
				...
				Accounts.publish( 'authorized', { 'data': params, 'socket': socket, 'type': type, 'response': profile, 'access_token': params.access_token, 'refresh_token': params.refresh_token } );
				...
				if( error ) {
					...
					service.google.get( "https://www.googleapis.com/plus/v1/people/me", access_token, function(error, data){
						if( error ) {
							...
							Accounts.publish( 'authorized', { 'data': params, 'socket': socket, 'type': type, 'response': parsed, 'access_token': access_token, 'refresh_token': refresh_token } );
