	...
	if( params && 'logout' == params.command ) {
		...
		service.yahoo.getOAuthAccessToken( params.oauth_token, params.oauth_token_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				//service.yahoo.get( "http://social.yahooapis.com/v1/me/guid", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
				service.yahoo.get( "http://social.yahooapis.com/v1/users.guid(" + oauth_access_token + ")/profile?format=json", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
