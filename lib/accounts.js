	...
	if( params && 'logout' == params.command ) {
		...
		service.yahoo.getOAuthAccessToken( params.oauth_token, params.oauth_token_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				service.yahoo.get( "http://social.yahooapis.com/v1/me/gu.id?format=json", oauth_access_token, oauth_access_token_secret, function ( error1, data1, result1 ) {
					
					var parsed1 = JSON.parse( data1 );
					var gu.id = parsed1.gu.id.value;

					service.yahoo.get( "http://social.yahooapis.com/v1/user/" + gu.id + "/profile?format=json", oauth_access_token, oauth_access_token_secret, function ( error, data, result ) {
