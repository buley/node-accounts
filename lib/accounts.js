	...
	if( params && 'logout' == params.command ) {
		...
		service.yahoo.getOAuthAccessToken( params.oauth_token, params.oauth_token_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				service.yahoo.get( "http://social.yahooapis.com/v1/me/guid?format=json", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
					console.log("SUER INFO",data);
					var parsed = JSON.parse( data );
					var guid = parsed.guid.value;

					service.yahoo.get( "http://social.yahooapis.com/v1/user/" + guid + "/profile", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
