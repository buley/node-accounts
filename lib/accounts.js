...
Private.handle_yahoo = function( params, callback ) {
	...
	if( params && 'logout' == params.command ) {
		...
		Private.publish( 'unauthorized', { 'data': params, 'callback': callback, 'type': type } );
	...
	if( params && 'logout' == params.command ) {
		...
		service.twitter.get( "https://api.twitter.com/1/account/verify_credentials.json", params.access_token, params.access_token_secret, function (error, data, result ) {
			...
			if( error ) { 
				...
				Private.publish( 'authorized', { 'data': params, 'response': parsed, 'callback': callback, 'type': type } );
		...
		service.twitter.getOAuthAccessToken(params.oauth_token, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				Private.publish( 'unauthorized', { 'data': params, 'callback': callback, 'type': type, 'error': error } );
				...
				service.twitter.get( "https://api.twitter.com/1/account/verify_credentials.json", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
					...
					Private.publish( 'authorized', { 'data': params, 'callback': callback, 'type': type, 'response': parsed, 'access_token': oauth_access_token, 'access_token_secret': oauth_access_token_secret } );
...
Private.handle_facebook = function( params, callback ) {
	...
	if( params && 'connect' == params.command ) {
		...
		Private.publish( 'authorizing', { 'data': params, 'callback': callback, 'type': type, 'authorization_url': redirect_url } );
