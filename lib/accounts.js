	...
	if( params && 'logout' === params.action ) {
        ...
        if ( 'GET' === method ) {
            ...
            service.linkedin.get( url, oauth_token, oauth_token_secret, function (error, data, result ) {
				console.log('linkedin', url, error, typeof data );
                if( error ) {
					success = false;
                }
                try {
    ...
    if( params && 'connect' === params.action ) {
        ...
        service.reddit.get( "https://oauth.reddit.com/api/me.json", params.access_token, function ( error, data, response ) {
            if( error ) {
				...
				console.log('getting subs',params.access_token);

				service.reddit.get( "https://oauth.reddit.com/reddits/mine/subscriber.json", params.access_token, function ( error, data, response2 ) {

                    console.log('reddit subs',params,error,data);
        ...
        if( params.error_reason == 'user_denied' ) {
            ...
            service.reddit.getOAuthAccessToken( params.code, {
                ...
                if( error ) {
					...
					console.log("GOT OAITJ". access_token);
                    service.reddit.get( "https://oauth.reddit.com/api/v1/me", access_token, function ( error, data, response ) {
                        if( error ) {
                            ...
                            service.reddit.get( "https://oauth.reddit.com/reddits/mine/subscriber.json?scope=mysubreddits", params.access_token, function ( error2, data2, response2 ) {
                                console.log('reddit subs',error2,data2);
