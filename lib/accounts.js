...
Private.handle_yahoo = function( params, callback, type ) {
...
/* Linkedin */
Private.handle_linkedin = function( params, callback, type ) {
	...
	if( params && 'logout' == params.command ) {
		...
		service.linkedin.getOAuthAccessToken( params.oauth_token, params.oauth_token_secret, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				console.log( "GETTING LINKEDIN: " + "http://api.linkedin.com/v1/people/~:(" + profile_params.join( ',' ) + ")");
...
/* Tumblr */
Private.handle_tumblr = function( params, callback, type ) {
...
Private.handle_twitter = function( params, callback, type ) {
...
Private.handle_facebook = function( params, callback, type ) {
	...
	if( params && 'connect' == params.command ) {
				...
				if( error ) { 
					...
					service.facebook.get( "https://graph.facebook.com/me", access_token, function ( error, data, response ) {
						if( error ) {
							...
							var result = { 'success': true, 'response_type': 'account', 'account_status': 'authorized', 'service': 'facebook', 'access_token': access_token, 'profile_data': parsed };
		...
		callback( { 'success': true, 'response_type': 'account', 'service': 'facebook', 'login_url': redirect_url } );

	}

};

Private.handle_windows = function( params, callback, type ) {
...
Private.handle_github = function( params, callback, type ) {
...
Private.handle_google = function( params, callback, type ) {
...
Private.handle_foursquare = function( params, callback, type ) {
...
Public.prototype.logout = function( data, callback, type ) {
	Private.publish( 'logging_out', { 'data': data, 'socket': socket, 'type': type } );
};

Public.prototype.login = function( data, callback, type ) {
	
	Private.publish( 'logging_in', { 'data': data, 'socket': socket, 'type': type } );
	
	if( 'twitter' == data.service ) {
		Private.handle_twitter( data, callback, type );
	} else if( 'google' === data.service ) {
		Private.handle_google( data, callback, type );
	} else if( 'facebook' === data.service ) {
		Private.handle_facebook( data, socket, type  );
	} else if( 'foursquare' === data.service ) {
		Private.handle_foursquare( data, callback, type );
	} else if( 'tumblr' === data.service ) {
		Private.handle_tumblr( data, callback, type );
	} else if( 'github' === data.service ) {
		Private.handle_github( data, callback, type );
	} else if( 'yahoo' === data.service ) {
		Private.handle_yahoo( data, callback, type );
	} else if( 'linkedin' === data.service ) {
		Private.handle_linkedin( data, callback, type );
	} else if( 'windows' === data.service ) {
		Private.handle_windows( data, callback, type );
	}
