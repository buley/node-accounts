...
Private.handle_vimeo = function( params, callback ) {
    ...
    if( params && 'logout' === params.action ) {
    	...
    	console.log("GET OAUTH VIMEO", error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters);
            ...
            if(error) {
                ...
                var authentication_url = "https://vimeo.com/oauth/authorize/?oauth_token=" + oauth_request_token;
...
Private.handle_facebook = function( params, callback ) {
	...
	if( params && 'connect' === params.action ) {
		...
		if( params.error_reason == 'user_denied' ) {
			...
			console.log("FACEBOOK CODE" + params.code);
...
Public.prototype.secrets = function( path ) {
    ...
    // Vimeo (oAuth 2)
    service.wordpress = new OAuth2( vimeo.id, vimeo.secret, "https://api.vimeo.com/", "/oauth2/authorize", "/oauth2/access_token" );
    service.wordpress.setAccessTokenName( vimeo.token || "oauth_token");
    service.wordpress.useAuthorizationHeaderforGET( true );
