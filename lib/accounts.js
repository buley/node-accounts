...
var Accounts = function( path ) {
	...
	if( 'undefined' === typeof path || null === path ) {
		path = '/var/www/wirecasts/oauth.secrets';
	}
...
Accounts.handle_twitter = function( params, socket ) {
	...
	if( params && 'logout' == params.command ) {
		...
		console.log( 'Retrieving request token', error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters );	
...
Public.prototype.logout = function( params ) {
	console.log('logging out');
};

Public.prototype.login = function( socket, data ) {
