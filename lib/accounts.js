...
Private.handle_windows = function( params, socket, type ) {
	...
	if( params && 'connect' == params.command ) {
		...
		service.windows.getProtectedResource( "https://apis.live.net/v5.0/me", params.access_token, function ( error, data, response ) {
				if( error ) {
		...
		if( params.error_reason == 'user_denied' ) {
			...
			service.windows.getOAuthAccessToken( params.code, { 'grant_type': 'authorization_code' }, function( error, access_token, refresh_token ){
				...
				if( error ) { 
					...
					service.windows.getProtectedResource( "https://apis.live.net/v5.0/me", access_token, function ( error, data, response ) {
		...
		var redirect_url = service.windows.getAuthorizeUrl( {
			redirect_uri: windowsCallbackAddress
			, response_type: "code"
		} );
...
Public.prototype.secrets = function( path ) {
	...
	// Linkedin (oauth 1.0)
	service.linkedin = new OAuth( "https://api.linkedin.com/uas/oauth/requestToken", "https://api.linkedin.com/uas/oauth/accessToken", linkedinId, linkedinSecret, "1.0", linkedinCallbackAddress || null, "HMAC-SHA1" );

	// Windows Live (oAuth 2)
	service.windows = new OAuth2( windowsId, windowsSecret, "https://oauth.live.com", "/authorize", "/token", "HMAC-SHA1" );
