...
Private.handle_tumblr = function( params, socket, type ) {
	...
	if( params && 'logout' == params.command ) {
		...
		service.tumblr.getOAuthAccessToken( params.oauth_token, params.oauth_verifier, function( error, oauth_access_token, oauth_access_token_secret, additionalParameters ) {
			...
			if( error ) {
				...
				service.tumblr.get( "https://www.tumblr.com/oauth", oauth_access_token, oauth_access_token_secret, function (error, data, result ) {
		...
		service.tumblr.getOAuthRequestToken( function( error, oauth_request_token, oauth_request_token_secret, oauth_authorize_url, additionalParameters ) {
			...
			if(error) {
				...
				var authentication_url = "http://tumblr.com/oauth/authenticate?oauth_token=" + oauth_request_token;
...
Public.prototype.secrets = function( path ) {
	...
	// Evernote
	//service.tumblr = new OAuth2( tumblrId, tumblrKey, "https://www.tumblr.com", "/oauth", "/oauth", "PLAINTEXT" );
	service.tumblr = new OAuth( "http://www.tumblr.com/oauth/request_token", "http://www.tumblr.com/oauth/access_token", tumblrId, tumblrKey, "1.0", tumblrCallbackAddress || null, "HMAC-SHA1" );
