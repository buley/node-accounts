...
Private.handle_reddit = function( params, callback ) {
	...
	if( params && 'proxy' === params.action ) {
    ...
    } else if( params && 'connect' === params.action && ( params.code || params.error_reason === 'user_denied' ) ) {
        ...
        if( params.error_reason == 'user_denied' ) {
			...
			console.log('getting access token for reddit',params.code);
            service.reddit.getOAuthAccessToken( params.code, {
                'grant_type': 'authorization_code'
                , redirect_uri: reddit.callback
            }, function( error, access_token, refresh_token ){
				console.log('requested reddit token', error, access_token);
                if( error ) {
